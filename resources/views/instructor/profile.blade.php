  ...
  <div class="panel-body">
    ...
    <form method="POST" action="{{ route('instructor.profile.save') }}" id="profileForm" enctype="multipart/form-data">
      ...
      <div class="row">
        ...
        <div class="form-group col-md-4">
            <label class="form-control-label">Pay <span class="required">*</span></label>
            <input type="text" class="form-control" name="pay_info" 
                placeholder="(Eg. KBZpay(09123456789))" value="{{ $instructor->pay_info }}" />
                @if ($errors->has('pay_info'))
                    <label class="error" for="pay_info">{{ $errors->first('pay_info') }}</label>
                @endif
        </div>
